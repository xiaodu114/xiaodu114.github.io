...
Write-Host "`t制 造 商：`t$($memory.Manufacturer)"
Write-Host "`t部 件 号：`t$($memory.PartNumber)"
Write-Host "`t序 列 号：`t$($memory.SerialNumber)"
Write-Host "`t容    量：`t$($memory.Capacity/1GB)GB"
Write-Host "`n"
...
if ($sounds -is [array]) {
	...
	Write-Host "Get-WmiObject -Class Win32_SoundDevice -ComputerName. | Select-Object -Property * 得到结果不是数组，还没有遇到这种情况……"
}
Write-Host "`n"
...
if ($nvdias -is [array]) {
	...
	Write-Host "Get-WmiObject -Class Win32_VideoController -ComputerName. | Select-Object -Property * 得到结果不是数组，还没有遇到这种情况……"
}
Write-Host "`n"
...
if ($netAdapters -is [array]) {
	...
	Write-Host "Get-NetAdapter | Select-Object -Property * 得到结果不是数组，还没有遇到这种情况……"
}
Write-Host "`n"
