customElements.define(
	...
	class MarkedBlock extends HTMLElement {
		...
		constructor() {
			...
			//  这里获取不到传入的属性值
			//  可以获取dom结构
			this.containerDom = this.shadowRoot.querySelector(
				".ddz-typesetting-markedblock-container"
			);
			this.markedblockExplainDom = this.shadowRoot.querySelector(
				".ddz-typesetting-markedblock-container>.ddz-typesetting-markedblock-explain"
			);
		}
		...
		connectedCallback() {
			let userSet_Type =
				this.hasAttribute("type") &&
					this.constructor.defaultTypeObj.hasOwnProperty(
			...
			for (const attr in customAttrObj) {
				if (Object.hasOwnProperty.call(customAttrObj, attr)) {
					if (!this.hasAttribute(attr)) {
						this.constructor.supportAttrObj[attr].update(
							this,
							customAttrObj[attr]
						);
					}
		...
		attributeChangedCallback(attrName, oldValue, newValue) {
			if (
				this.constructor.supportAttrObj.hasOwnProperty(attrName) &&
				newValue
			) {
...
customElements.define(
	...
	class BackToTop extends HTMLElement {
		constructor() {
			...
			this.backtotopTextDom = this.shadowRoot.querySelector(
				".ddz-typesetting-backtotop-container>.ddz-typesetting-backtotop-text"
			);
		...
		connectedCallback() {
			...
			if (scrollSelectorValue == null) {
				...
				if (
					...
					typeof document.body.parentNode.scrollTop === "number"
				) {
				...
				if (!this.monitorEle)
					throw new Error(`根据${scrollSelectorValue}未能找到元素！`);
		 ...
		 */
		disconnectedCallback() {
			this.monitorEle.removeEventListener(
				"scroll",
				this.handleMonitorEleScroll
			);
		...
		handleMonitorEleScroll(evt) {
			if (
				!(
					typeof this.scrollEle.scrollTop === "number" &&
					typeof this.scrollEle.scrollHeight === "number" &&
					typeof this.scrollEle.clientHeight === "number"
				)
			)
				return;
			if (this.scrollEle.scrollTop === 0) {
				...
				this.backtotopTextDom.innerHTML =
					(
...
customElements.define(
	...
	class SingleSlider extends HTMLElement {
		...
		constructor() {
			...
			this.loadDom = this.shadowRoot.querySelector(
				".single-slider-container .load-progress"
			);
			this.slidDom = this.shadowRoot.querySelector(
				".single-slider-container .slid-progress"
			);
			this.handlerDom = this.shadowRoot.querySelector(
				".single-slider-container .slider-handler"
			);
		}
		...
		attributeChangedCallback(attrName, oldValue, newValue) {
			if (oldValue !== newValue && newValue) {
				if (
					["dot-radius", "dot-size", "slider-height"].indexOf(attrName) >= 0
				) {
		...
		set value(newValue) {
			...
			if (
				...
				if (
					Object.hasOwnProperty.call(newValue, "slidValue") &&
					!isNaN(Number(newValue["slidValue"])) &&
					Number(newValue["slidValue"]) > 0
				) {
					tempSlidValue = newValue["slidValue"];
				}
				if (
					Object.hasOwnProperty.call(newValue, "loadValue") &&
					!isNaN(Number(newValue["loadValue"])) &&
					Number(newValue["loadValue"]) > 0
				) {
			...
			if (
				newValue.loadValue !== oldValue.loadValue ||
				newValue.slidValue !== oldValue.slidValue
			) {
				this.dispatchEvent(
					new CustomEvent("change", {
						detail: {
...
customElements.define(
	...
	class ScaleClock extends HTMLElement {
		constructor() {
			...
			this.clockContainerDom =
				this.shadowRoot.querySelector(".clock-container");
			this.clockCenterAxisContainerDom = this.shadowRoot.querySelector(
				".clock-center-axis-container"
			);
			this.clockCenterPointDom = this.shadowRoot.querySelector(
				".clock-center-point"
			);
		...
		connectedCallback() {
			...
			//  1、钟表最外层容器
			this.clockContainerDom.style.height = this.clockContainerDom.style.width =
				this.defaultClockOptions.radius * 2 + "px";

			//  2、钟表中心圆点容器（容器中放置：中心圆片、时针、分针、秒针）
			this.clockCenterAxisContainerDom.style.left =
				this.clockCenterAxisContainerDom.style.top =
				this.defaultClockOptions.radius + "px";
			//      2.1、钟表中心圆点容器之 中心圆片
			this.clockCenterPointDom.style.height =
				this.clockCenterPointDom.style.width =
				this.defaultClockOptions.centerPoint.size + "px";
			this.clockCenterPointDom.style.left = this.clockCenterPointDom.style.top =
				0 - this.defaultClockOptions.centerPoint.size / 2 + "px";
			//      2.2、钟表中心圆点容器之 时针
			let hourHandTailLength = this.defaultClockOptions.radius * 0.1,
				hourHandTransformDistance =
					this.defaultClockOptions.radius -
					this.defaultClockOptions.num60.size *
					this.defaultClockOptions.num60.isShow -
					this.defaultClockOptions.scale.length -
					this.defaultClockOptions.num12.size;
			this.hourHandDom.style.width =
				this.defaultClockOptions.hourHand.width + "px";
			this.hourHandDom.style.height =
				hourHandTransformDistance + hourHandTailLength + "px";
			this.hourHandDom.style.left =
				0 - this.defaultClockOptions.hourHand.width / 2 + "px";
			this.hourHandDom.style.bottom = 0 - hourHandTailLength + "px";
			this.hourHandDom.style.transformOrigin =
				"center " + hourHandTransformDistance + "px";
			//      2.3、钟表中心圆点容器之 分针
			let minuteHandTailLength = this.defaultClockOptions.radius * 0.15,
				minuteHandTransformDistance =
					this.defaultClockOptions.radius -
					this.defaultClockOptions.num60.size *
					this.defaultClockOptions.num60.isShow -
					this.defaultClockOptions.scale.length;
			this.minuteHandDom.style.width =
				this.defaultClockOptions.minuteHand.width + "px";
			this.minuteHandDom.style.height =
				minuteHandTransformDistance + minuteHandTailLength + "px";
			this.minuteHandDom.style.left =
				0 - this.defaultClockOptions.minuteHand.width / 2 + "px";
			this.minuteHandDom.style.bottom = 0 - minuteHandTailLength + "px";
			this.minuteHandDom.style.transformOrigin =
				"center " + minuteHandTransformDistance + "px";
			//      2.4、钟表中心圆点容器之 秒针
			if (this.defaultClockOptions.secondHand.isShow) {
				...
				if (this.defaultClockOptions.num60.isShow) {
					secondHandTransformDistance -=
						this.defaultClockOptions.num60.size / 2;
				} else {
					secondHandTransformDistance -=
						this.defaultClockOptions.scale.length / 2;
				}
				this.secondHandDom.style.width =
					this.defaultClockOptions.secondHand.width + "px";
				this.secondHandDom.style.height =
					secondHandTransformDistance + secondHandTailLength + "px";
				this.secondHandDom.style.left =
					0 - this.defaultClockOptions.secondHand.width / 2 + "px";
				this.secondHandDom.style.bottom = 0 - secondHandTailLength + "px";
				this.secondHandDom.style.transformOrigin =
					"center " + secondHandTransformDistance + "px";
			}
			...
			clockScaleDialContainerDom.style.left =
				clockScaleDialContainerDom.style.top =
				this.defaultClockOptions.radius + "px";
			//      3.2、12数字盘：1-12
			...
			clockNum12DialContainerDom.style.left =
				clockNum12DialContainerDom.style.top =
				this.defaultClockOptions.radius + "px";
			//      3.3、60数字盘：1-60
			...
			clockNum60DialContainerDom.style.left =
				clockNum60DialContainerDom.style.top =
				this.defaultClockOptions.radius + "px";
			...
			for (let index = 1; index <= 60; index++) {
				...
				if (this.defaultClockOptions.num60.isShow) {
					...
					numItemContentDomFor60.style.height =
						numItemContentDomFor60.style.width =
						this.defaultClockOptions.num60.size + "px";
					numItemContentDomFor60.style.top = numItemContentDomFor60.style.left =
						0 - this.defaultClockOptions.num60.size / 2 + "px";
					numItemContentDomFor60.style.fontSize =
						this.defaultClockOptions.num60.size + "px";
					let numItemDomFor60 = document.createElement("div");
					numItemDomFor60.classList.add("clock-num-60-item-base-container");
					numItemDomFor60.style.top =
						Math.sin(needCalcDeg) * this.defaultClockOptions.radius + "px";
					numItemDomFor60.style.left =
						Math.cos(needCalcDeg) * this.defaultClockOptions.radius + "px";
				...
				let scaleItemContentDom = document.createElement("div"),
					scaleItemContentHeight =
						index % 5 === 0 ?
							this.defaultClockOptions.scale.bigWidth :
							this.defaultClockOptions.scale.smallWidth;
				scaleItemContentDom.classList.add("scale-content");
				scaleItemContentDom.style.width =
					this.defaultClockOptions.scale.length + "px";
				scaleItemContentDom.style.height = scaleItemContentHeight + "px";
				scaleItemContentDom.style.marginTop =
					0 - scaleItemContentHeight / 2 + "px";
				let scaleItemDom = document.createElement("div");
				scaleItemDom.classList.add("clock-scale-item-base-container");
				scaleItemDom.classList.add(
					index % 5 === 0 ? "big-scale" : "small-scale"
				);
				scaleItemDom.style.transform = `rotate(${90 + index * 6}deg)`;
				scaleItemDom.style.top =
					Math.sin(needCalcDeg) *
					(this.defaultClockOptions.radius -
						this.defaultClockOptions.num60.size *
						this.defaultClockOptions.num60.isShow) +
					"px";
				scaleItemDom.style.left =
					Math.cos(needCalcDeg) *
					(this.defaultClockOptions.radius -
						this.defaultClockOptions.num60.size *
						this.defaultClockOptions.num60.isShow) +
					"px";
				...
				if (index % 5 === 0) {
					...
					numItemContentDomFor12.style.height =
						numItemContentDomFor12.style.width =
						this.defaultClockOptions.num12.size + "px";
					numItemContentDomFor12.style.top = numItemContentDomFor12.style.left =
						0 - this.defaultClockOptions.num12.size / 2 + "px";
					numItemContentDomFor12.style.fontSize =
						this.defaultClockOptions.num12.size + "px";
					let numItemDom = document.createElement("div");
					numItemDom.classList.add("clock-num-12-item-base-container");
					numItemDom.style.top =
						Math.sin(needCalcDeg) *
						(this.defaultClockOptions.radius -
							(this.defaultClockOptions.num60.isShow ? 1 : 2) *
							this.defaultClockOptions.scale.length -
							2 *
							this.defaultClockOptions.num60.size *
							this.defaultClockOptions.num60.isShow) +
						"px";
					numItemDom.style.left =
						Math.cos(needCalcDeg) *
						(this.defaultClockOptions.radius -
							(this.defaultClockOptions.num60.isShow ? 1 : 2) *
							this.defaultClockOptions.scale.length -
						...
						case "size": {
							if (!isNaN(Number(attrValue))) {
								this.clockCenterPointDom.style.height =
									this.clockCenterPointDom.style.width =
									Number(attrValue) + "px";
								this.clockCenterPointDom.style.left =
									this.clockCenterPointDom.style.top =
									0 - Number(attrValue) / 2 + "px";
							}
							break;
						}
						case "style": {
		...
		_BatchUpdateEleStyleByCustom(selectors, eleAndStyleObj) {
			if (
				this._GetDataType(eleAndStyleObj) === "object" &&
				Object.getOwnPropertyNames(eleAndStyleObj).length
			) {
		...
		_UpdateEleStyle(targetEle, styleObj) {
			if (this._GetDataType(styleObj) === "object") {
				for (const key in styleObj) {
					if (
						Object.hasOwnProperty.call(styleObj, key) &&
		...
		_Update3HandPos(currentDateTime) {
			...
			this.defaultClockOptions.secondHand.isShow &&
				(this.secondHandDom.style.transform = `rotate(${currentSecond * 6
					}deg)`);
			this.minuteHandDom.style.transform = `rotate(${currentMinute * 6 + 0.1 * currentSecond
				}deg)`;
			this.hourHandDom.style.transform = `rotate(${currentHour * 30 + (currentMinute * 60 + currentSecond) / 120
				}deg)`;
		}
...
customElements.define(
	...
	class AutoGenerateDirectory extends HTMLElement {
		constructor() {
			...
			this.triggerDom = this.shadowRoot.querySelector(
				".ddz-auto-generate-show-directory-trigger"
			);
			this.triggerCoverDom = this.shadowRoot.querySelector(
				".ddz-auto-generate-show-directory-trigger-cover"
			);
			this.showModeDom = this.shadowRoot.querySelector(
				".ddz-auto-generate-directory-show-mode"
			);
			...
			this.listenerTriggerClickEvent =
				this.listenerTriggerClickEvent.bind(this);
			this.listenerTriggerCoverClickEvent =
				this.listenerTriggerCoverClickEvent.bind(this);
			...
			this.triggerCoverDom.addEventListener(
				"click",
				this.listenerTriggerCoverClickEvent
			);
		}
		 ...
		 */
		disconnectedCallback() {
			this.triggerDom.removeEventListener(
				"click",
				this.listenerTriggerClickEvent
			);
			this.triggerCoverDom.removeEventListener(
				"click",
				this.listenerTriggerCoverClickEvent
			);
		...
		getDirectorySourceHDoms() {
			if (!this.sourceHDoms || !this.sourceHDoms.length) {
				this.sourceHDoms = document.querySelectorAll(
					["h1", "h2", "h3", "h4", "h5", "h6"]
						.map((tag) => this.sourceSelector + ">" + tag)
						.join(",")
				);
			}
		...
		listenerScrollEvent() {
			...
			[].forEach.call(
				this.containerDom.querySelectorAll(".selected") || [],
				(itemDom) => {
			...
			if (priority1Dom) {
				this.containerDom
					.querySelector(`[href='#${priority1Dom.id}']`)
					.classList.add("selected");
			} else if (priority2Dom) {
				this.containerDom
					.querySelector(`[href='#${priority2Dom.id}']`)
					.classList.add("selected");
			}
		}
		...
		init() {
			...
			this.containerDom.classList.add(
				"ddz-auto-generate-directory-items-container"
			);
			...
			if (
				(viewingAreaWidth - tempSourceDomWidth - scrollBarWidth) / 2 -
				containerMarginLR >
				containerWidth
			) {
				this.containerDom.style.right =
					((viewingAreaWidth - tempSourceDomWidth - scrollBarWidth) / 2 -
						containerWidth) /
					2 +
					"px";
...
customElements.define(
	...
	class MyDialog extends HTMLElement {
		...
		constructor() {
			...
			//  这里获取不到传入的属性值
			//  可以获取dom结构
			this.maskLayerDom = this.shadowRoot.querySelector(
				".ddz-my-dialog-mask-layer"
			);
			this.myDialogContentDom = this.shadowRoot.querySelector(
				".ddz-my-dialog-mask-layer>.my-dialog-content"
			);
			this.myDialogTitleDom = this.shadowRoot.querySelector(
				".ddz-my-dialog-mask-layer>.my-dialog-content .my-dialog-content-header>.my-dialog-default-title"
			);
			this.myDialogCloseDom = this.shadowRoot.querySelector(
				".ddz-my-dialog-mask-layer>.my-dialog-content .my-dialog-content-header>.my-dialog-default-close"
			);
