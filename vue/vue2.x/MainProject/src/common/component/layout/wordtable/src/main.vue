  ...
  methods: {
    getGUID() {
      return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(
        ...
        function (c) {
          ...
          return ("x" == c ? r : (3 & r) | 8).toString(16).toUpperCase();
        }
      );
    },
    getDataType(obj) {
    ...
    undoOperate() {
      if (this.wordTableCellsHistory.length > 1) {
        ...
        this.wordTableCells = cloneDeep(
          this.wordTableCellsHistory[this.wordTableCellsHistory.length - 1]
        );
      }
    },
    //  ctrl+y撤销操作
    ...
    quickInsertColumnCircleClickToInsertOneColumn(cell) {
      ...
      var tempRowEnds = Array.from(
        new Set(this.wordTableCells.map((x) => x.rowEnd).sort((a, b) => a - b))
      );
    ...
    judgeSelectedCellsShapeIsValid() {
      ...
      this.tempSelectedCellsToMerge.map((x) => {
        minRowStart = minRowStart > x.rowStart ? x.rowStart : minRowStart;
        minColumnStart =
          minColumnStart > x.columnStart ? x.columnStart : minColumnStart;
        maxRowEnd = maxRowEnd < x.rowEnd ? x.rowEnd : maxRowEnd;
        maxColumnEnd = maxColumnEnd < x.columnEnd ? x.columnEnd : maxColumnEnd;
        selectedCellsArea +=
    ...
    judgeIsShowSplitCell(cell, index) {
      ...
      retObj.isShow =
        cell.columnEnd - cell.columnEnd > 1 || cell.rowEnd - cell.rowStart > 1;
    ...
    contextMenuItemInsertRowFormBottom(extendParameter) {
      this.quickInsertRowCircleClickToInsertOneRow(extendParameter.cellItem);
    },
    contextMenuItemDeleteCellWholeRow(extendParameter) {
      var tempRowCounter = extendParameter.rowEnd - extendParameter.rowStart;
    ...
    contextMenuItemMergeCell(extendParameter) {
      ...
      for (let i = 0; i < this.wordTableCells.length; ) {
        if (
          this.tempSelectedCellsToMerge.indexOf(this.wordTableCells[i]) >= 0
        ) {
          this.wordTableCells.splice(i, 1);
        } else i++;
      }
    ...
    cellRightClickToShowContextMenu(cell, index, event) {
      ...
      this.allContextmenuItems.forEach((item) => {
        if (
          ...
          this.getDataType(this[item.judgeAvailableMethod]) === "function"
        ) {
          var tempJudgeResult = this[item.judgeAvailableMethod](cell, index);
